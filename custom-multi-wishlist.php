...
class Custom_Multi_Wishlist {
	...
	private function init_hooks() {
		...
		// Replace template_redirect with content filter to avoid breaking Elementor
		add_filter('the_content', [$this, 'filter_wishlist_content'], 20);
	}
	...
	private function load_user_wishlists() {
		...
		if (empty($wishlists_data) || !is_array($wishlists_data)) {
			// Do not auto-create a default wishlist; start with no wishlists
			$this->wishlists = [];
		} else {
			// Remove any legacy default entries and store back if changed
			$original = $wishlists_data;
			foreach ($wishlists_data as $id => $wl) {
				$wl_id = isset($wl['id']) ? $wl['id'] : $id;
				$is_default = !empty($wl['is_default']);
				if ($wl_id === 'default' || $is_default) {
					unset($wishlists_data[$id]);
				}
			}
			if ($wishlists_data !== $original) {
				update_user_meta($user_id, 'custom_multi_wishlists', $wishlists_data);
			}
	...
	public function get_user_wishlists() {
		// Hide any legacy/default wishlists from all UI
		$filtered = [];
		foreach ((array) $this->wishlists as $id => $wishlist) {
			$wl_id = isset($wishlist['id']) ? $wishlist['id'] : $id;
			$is_default = !empty($wishlist['is_default']);
			if ($wl_id === 'default' || $is_default) {
				continue;
			}
			$filtered[$wl_id] = $wishlist;
		}
		return $filtered;
	}
	...
	public function get_wishlist($wishlist_id) {
		if ($wishlist_id === 'default') {
			return null; // Block access to default wishlist everywhere
		}
		if (!isset($this->wishlists[$wishlist_id])) {
			return null;
		}
		$wishlist = $this->wishlists[$wishlist_id];
		if (!empty($wishlist['is_default'])) {
			return null;
		}
		return $wishlist;
	}
	...
	public function add_rewrite_rules() {
		...
		add_rewrite_rule('^wishlist/([^/]+)/?$', 'index.php?pagename=wishlist&wishlist_id=$matches[1]', 'top');

		// Map /all-wishlists to a page with slug 'all-wishlists'
		add_rewrite_rule('^all-wishlists/?$', 'index.php?pagename=all-wishlists', 'top');
	}
	...
	/**
	 * Filter wishlist page content for /wishlist/{id}
	 */
	public function filter_wishlist_content($content) {
		...
		if (!$wishlist_id) {
			return $content; // Regular Jet widget content for /wishlist
		}

		// Render our custom wishlist content without breaking Elementor
		$wishlist = $this->get_wishlist($wishlist_id);
		if (!$wishlist) {
			return '<p>Wishlist not found.</p>';
		}

		return do_shortcode('[custom_multi_wishlist wishlist_id="' . esc_attr($wishlist_id) . '" show_actions="true"]');
	}
}
